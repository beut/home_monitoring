...
- [X] T001 Create the Gradle project skeleton:
...
- [X] T002 Generate the Gradle wrapper for 8.7 in `android/` by running `gradle wrapper --gradle-version 8.7` (produces `android/gradlew`, `android/gradlew.bat` and `android/gradle/wrapper/*`).
- [X] T003 Create `android/app/build.gradle.kts`:
...
- [X] T004 [P] Debug-only credential prefill in `android/app/build.gradle.kts` (research R6):
...
- [X] T005 [P] Create `android/app/src/main/AndroidManifest.xml`:
...
- [X] T006 [P] Create the backup exclusions so encrypted credentials and the DataStore files are never backed up:
...
- [X] T007 [P] Create the base resources:
...
- [X] T009 [P] Signer test in `android/app/src/test/java/pl/home/monitoring/data/apsystems/ApsSignerTest.kt`:
...
- [X] T010 [P] DTO parsing tests in `android/app/src/test/java/pl/home/monitoring/data/apsystems/ApsDtosTest.kt`, using the literal JSON fixtures from `contracts/apsystems-openapi.md`:
...
- [X] T011 [P] API client error-mapping tests with MockWebServer in `android/app/src/test/java/pl/home/monitoring/data/apsystems/ApsApiClientTest.kt`:
  - The request carries all 5 `X-CA-*` headers, and the nonce matches `^[0-9a-f]{32}$`.
  - `code 0` → Success; `1001` → NoData; `4000` → AuthError; `2001` and `4001` → AuthError; `2005` → Throttled; `7002` → Throttled; `5000` → ServiceError(5000).
...
- [X] T012 [P] SunCalculator tests in `android/app/src/test/java/pl/home/monitoring/core/SunCalculatorTest.kt`. For lat 52.0, lon 19.0 and zone Europe/Warsaw, the results must match these reference values within ±5 min:
  - 2026-06-21: sunrise 04:23, sunset 21:07.
  - 2026-09-23: sunrise 06:30, sunset 18:41.
  - 2026-12-21: sunrise 07:49, sunset 15:34.
  - These were cross-checked with the independent USNO almanac algorithm, which gives the published Warsaw times of 04:14/21:01 and 07:42/15:25.
- [X] T013 [P] RefreshPolicy tests in `android/app/src/test/java/pl/home/monitoring/core/RefreshPolicyTest.kt`, with a fixed clock and the data-model.md rules:
...
- [X] T014 [P] Formatter tests in `android/app/src/test/java/pl/home/monitoring/core/FormattersTest.kt`:
...
- [X] T015 [P] Domain types in `android/app/src/main/java/pl/home/monitoring/domain/Models.kt`:
...
- [X] T016 [P] `ApsSigner` in `android/app/src/main/java/pl/home/monitoring/data/apsystems/ApsSigner.kt`:
  ...
  - T009 must pass.
- [X] T017 [P] DTOs in `android/app/src/main/java/pl/home/monitoring/data/apsystems/ApsDtos.kt`:
  ...
  - T010 must pass.
- [X] T018 `CallBudgetStore` in `android/app/src/main/java/pl/home/monitoring/data/store/CallBudgetStore.kt`, backed by DataStore Preferences (file `budget`):
...
- [X] T019 `ApsApiClient` in `android/app/src/main/java/pl/home/monitoring/data/apsystems/ApsApiClient.kt`:
  ...
  - The code mapping follows contracts/apsystems-openapi.md: 0 → Success, 1001 → NoData, 2005/7001/7002/7003 → Throttled, other 2000–2999 and 4000–4999 → AuthError, anything else → ServiceError.
  ...
  - T011 must pass.
- [X] T020 [P] `SunCalculator` in `android/app/src/main/java/pl/home/monitoring/core/SunCalculator.kt`:
...
- [X] T021 `RefreshPolicy` in `android/app/src/main/java/pl/home/monitoring/core/RefreshPolicy.kt`. It is a pure function, `decide(now: Instant, zone: ZoneId, lat, lon, budget: CallBudget, snapshot: PvSnapshot?, manual: Boolean): RefreshDecision(fetchMinutely: Boolean, fetchSummary: Boolean, skipReason: SkipReason?, nextScheduledAt: Instant)`, where `SkipReason ∈ Night | TooSoon | BudgetExhausted | Throttled`. It implements the data-model.md rules verbatim:
  ...
  - T013 must pass.
- [X] T022 [P] Formatters in `android/app/src/main/java/pl/home/monitoring/core/Formatters.kt`:
  ...
  - Use `Locale("pl", "PL")`.
  - T014 must pass.
- [X] T023 [P] `CredentialCipher` in `android/app/src/main/java/pl/home/monitoring/data/security/CredentialCipher.kt`:
...
- [X] T024 `CredentialsStore` in `android/app/src/main/java/pl/home/monitoring/data/store/CredentialsStore.kt`, backed by DataStore (file `credentials`):
...
- [X] T025 [P] `SystemInfoStore` and `LocationStore` in `android/app/src/main/java/pl/home/monitoring/data/store/SettingsStores.kt`, backed by DataStore (file `settings`):
...
- [X] T028 [P] [US1] Repository tests in `android/app/src/test/java/pl/home/monitoring/data/apsystems/PvRepositoryTest.kt`, using a fake `ApsApiClient`, in-memory stores and a fixed clock:
...
- [X] T029 [US1] `PvRepository` in `android/app/src/main/java/pl/home/monitoring/data/apsystems/PvRepository.kt`:
  ...
  - T028 must pass.
- [X] T030 [US1] `DashboardViewModel` in `android/app/src/main/java/pl/home/monitoring/ui/dashboard/DashboardViewModel.kt`:
...
- [X] T031 [P] [US1] `PvSection` composable in `android/app/src/main/java/pl/home/monitoring/ui/dashboard/PvSection.kt`. It is an `ElevatedCard` titled "Fotowoltaika" with a solar icon, and it renders these states per contracts/ui-screens.md:
...
- [X] T032 [US1] `DashboardScreen` in `android/app/src/main/java/pl/home/monitoring/ui/dashboard/DashboardScreen.kt`:
...
- [X] T033 [US1] Wire `MainActivity` in `android/app/src/main/java/pl/home/monitoring/MainActivity.kt`:
...
- [X] T034 [P] [US2] Tests in `android/app/src/test/java/pl/home/monitoring/ui/accounts/AccountsViewModelTest.kt`, with a fake API client and in-memory stores:
...
- [X] T035 [US2] `AccountsViewModel` in `android/app/src/main/java/pl/home/monitoring/ui/accounts/AccountsViewModel.kt`:
  ...
  - T034 must pass.
- [X] T036 [US2] `AccountsScreen` in `android/app/src/main/java/pl/home/monitoring/ui/accounts/AccountsScreen.kt`:
...
- [X] T037 [US2] Start routing and navigation in `android/app/src/main/java/pl/home/monitoring/MainActivity.kt`:
...
- [X] T038 [US2] In `android/app/src/main/java/pl/home/monitoring/ui/dashboard/PvSection.kt`, wire the `NotConnected` state's "Podłącz konto" button to navigate to Accounts. This covers User Story 2, acceptance scenario 4.
- [X] T039 [US2] Remove the temporary debug auto-save from T033 in `android/app/src/main/java/pl/home/monitoring/MainActivity.kt`. From now on the dev prefill only fills the form.
...
- [X] T040 [P] [US3] Extend `android/app/src/test/java/pl/home/monitoring/data/apsystems/PvRepositoryTest.kt` with these cases:
  - (g) Offline with a snapshot → `Data(Stale(Offline))`, and the snapshot values are unchanged.
  - (h) Throttled: each of the codes 2005, 7001, 7002 and 7003 → `Data(Stale(Throttled))`. `budget.throttledUntil` is set to the next scheduled slot (at least 15 min ahead) and doubles on repeats (capped at 6 h). **No** code blocks the rest of the month. The next automatic refresh before `throttledUntil` makes **no** call.
  ...
  - (k) Minutely returns AuthError and `details` succeeds with a different ECU id → the new id is saved and minutely is retried once (at most 2 extra calls). If `details` also returns AuthError → the state is `AuthError`.
  - (l) After a failed attempt (Offline), the next automatic refresh within `interval` makes no call (it uses `lastAttemptAt`).

### Implementation for User Story 3

- [X] T041 [US3] Refine error handling in `android/app/src/main/java/pl/home/monitoring/data/apsystems/PvRepository.kt`:
  ...
  - T040 must pass.
- [X] T042 [P] [US3] Render the stale and error states in `android/app/src/main/java/pl/home/monitoring/ui/dashboard/PvSection.kt`, per contracts/ui-screens.md:
...
- [X] T043 [US3] Wire `onRetry` in `android/app/src/main/java/pl/home/monitoring/ui/dashboard/DashboardViewModel.kt` and `android/app/src/main/java/pl/home/monitoring/ui/dashboard/DashboardScreen.kt`. Make sure the other sections (future sources) are refreshed independently, so one source's failure or exception never cancels another's. Use `supervisorScope` in `onPullToRefresh` and in the auto-refresh loop (FR-012).
...
- [X] T044 [P] Move every user-visible Polish string from the composables and view models into `android/app/src/main/res/values/strings.xml`, and reference it via `stringResource` / `context.getString` (FR-016).
- [X] T045 [P] Harden logging:
...
- [X] T046 [P] Release build configuration in `android/app/build.gradle.kts`: `isMinifyEnabled = true` with `proguard-android-optimize.txt`, and add the kotlinx.serialization keep rules to `android/app/proguard-rules.pro`. Verify that `./gradlew assembleRelease` succeeds (an unsigned build is fine).
- [X] T047 [P] Add `android/README.md` (in Polish) covering:
  - How to build.
...
- Implementation note (2026-09-23): US1 and US2 were built in the same run, so the temporary debug auto-save (T033) was never needed and T039 had nothing to remove. The dev prefill only fills the Konta form.
